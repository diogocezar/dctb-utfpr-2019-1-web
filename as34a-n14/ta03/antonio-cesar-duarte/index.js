window.onload = function(e){
	...
	.then(function(json){
		var configs = document.getElementsByClassName("my-infos");
		var products = document.getElementsByClassName("my-row");
	...
	const fillProducts = function(p){
		...
		if(active){
			var element = `<div class="item">${name} ${img} ${desc} ${price} ${comprar}</div>`;
			return element;
		}else{
			var element = `<div class="itemF">${name} ${img} ${desc} ${price} ${comprar}</div>`;
			return element;
		}
	}
