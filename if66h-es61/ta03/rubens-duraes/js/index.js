...
$(document).ready(() => {
    ...
    axios.get(url).then(response => {
        ...
        var configs = criaConfig(config);
        $('#topo').html(configs);

        var html = lista(produtos);
        $('#produtos').html(html);
...
function lista(produtos) {
    ...
    for (var i = 0; i < produtos.length; i++) {
        if(!produtos[i].active) mudaBg = "cinza";
        html.push('<div class="col-sm">');
        html.push('<div class="card '+ mudaBg+'" style="width: 18rem;">');
